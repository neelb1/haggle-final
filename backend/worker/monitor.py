...
import config
from services import airbyte_service, overshoot_service, senso_service, tavily_service, gmail_service

logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s %(levelname)s %(name)s: %(message)s",
)
logger = logging.getLogger("haggle.monitor")
...
async def run_scan() -> list[dict]:
    ...
    # Notify user of detections — Slack + Gmail
    if detections:
        if config.SLACK_BOT_TOKEN:
            msg = f"Haggle Monitor: {len(detections)} anomalies detected in latest scan"
            await airbyte_service.send_slack_alert(msg)
        # Send rich threat alert email with price change details and dashboard link
...
async def main():
    ...
    logger.info("Haggle Monitor starting up")
